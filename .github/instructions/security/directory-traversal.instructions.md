...
3. 根本的解決が未実施の場合、`SafeFilePath` + `FileStorageUtil` による保険的対策が実施されているか → 実施されていれば保険的対策済みとし、根本的解決の検討を併せて促す
4. 独自にパス検証を実装しているか → `SafeFilePath` + `FileStorageUtil` への移行を必須として指摘し、根本的解決の検討も併せて促す
5. いずれも未実施か → 脆弱性リスクを明示し、根本的解決を強く求めるとともに、最低限 `SafeFilePath` + `FileStorageUtil` による保険的対策の即時実施を求める

## レビュー観点

### 根本的解決（最も推奨される対策。以下が実施されている場合、指摘不要）
...
### 保険的対策（根本的解決ができない場合、必ず実施すること）

根本的解決が実施できない場合は、プロジェクト内の `SafeFilePath` + `FileStorageUtil` による保険的対策を必ず実施すること。保険的対策として認められるのは、`SafeFilePath` + `FileStorageUtil` を利用している場合のみとする。
...
- `SafeFilePath` + `FileStorageUtil` を利用している場合でも、あくまで保険的対策であるため、根本的解決（ID間接参照等による外部入力のファイルパス排除）の検討を併せて促すこと

### 独自実装による対策（`SafeFilePath` + `FileStorageUtil` への移行を指摘する）

以下のように独自にパス検証を実装している場合は、`SafeFilePath` + `FileStorageUtil` への移行を指摘すること。

- ファイル名のチェック（`/`、`../`、`..\` 等の検出）を独自に実装している
  - URLデコード後の `%2F`、`..%2F`、`..%5C` や二重エンコードの `%252F`、`..%252F`、`..%255C` の考慮漏れが発生しやすい
- `Path.normalize()` + `Path.startsWith()` 等のパス検証を独自に実装している
- ファイルへのアクセス権限の設定のみで対処している
- 独自実装はプロジェクト内で検証ロジックが分散し、対策漏れや不整合の原因となるため、`SafeFilePath` + `FileStorageUtil` への移行を必須として指摘すること
- 併せて、根本的解決（ID間接参照等による外部入力のファイルパス排除）の検討も促すこと
...
- `SafeFilePath` + `FileStorageUtil` による保険的対策のみで対処している場合は、根本的解決の実施を検討するよう指摘する
- 独自にパス検証を実装している場合は、`SafeFilePath` + `FileStorageUtil` への移行を指摘し、根本的解決の検討も併せて促す
- 根本的解決・保険的対策のいずれも未実施の場合は、脆弱性のリスクを明示し、根本的解決を強く求めるとともに、最低限 `SafeFilePath` + `FileStorageUtil` による保険的対策の即時実施を求める
