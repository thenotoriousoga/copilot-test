...
3. `SafeFilePath` + `FileStorageUtil` による保険的対策が実施されているか → 実施されていれば、根本的解決の検討を併せて促す
4. 独自にパス検証を実装しているか → `SafeFilePath` + `FileStorageUtil` への移行を指摘し、根本的解決の検討も併せて促す
5. いずれも未実施か → 脆弱性リスクを明示し、根本的解決を強く求める
...
### 保険的対策（`SafeFilePath` + `FileStorageUtil` を利用している場合）

保険的対策として認められるのは、プロジェクト内の `SafeFilePath` + `FileStorageUtil` を利用している場合のみとする。
...
- 独自実装はプロジェクト内で検証ロジックが分散し、対策漏れや不整合の原因となるため、`SafeFilePath` + `FileStorageUtil` に統一すること
